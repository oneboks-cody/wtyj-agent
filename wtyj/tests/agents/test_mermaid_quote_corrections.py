...
@pytest.mark.parametrize('new_fields', [{'adults':3}, {'trip_date':'2027-09-10'}])
def test_change_followup_rebuilds_quote(monkeypatch, tmp_path, new_fields):
 ...
 monkeypatch.setattr(marina_agent,'process_message',Mock(return_value=decision('details',fields=new_fields,reply='Ik pas het aan.')))
 ...
 assert all(updated['intake'][k]==v for k,v in new_fields.items())
 ...
 # The replacement must still receive a separate approval before checkout.
 monkeypatch.setattr(marina_agent,'process_message',Mock(return_value=decision('confirm_summary')))
 approved=flow.handle_demo_message({'from':'guest','_zernio_account_id':'account','message_id':'new-approval','text':'De nieuwe offerte klopt'},True,use_model=True)
 assert '/pay/' in approved['text']
