...
SCRIPT = Path(__file__).resolve().parents[2] / 'scripts/recover_isluno_staged_cutover.py'
...
class RecoveryTests(unittest.TestCase):
    ...
    def test_wrong_cas_or_binding_never_quarantines(self):
        ...
        self.assertFalse((self.root/'state_registry.recovery-d.sqlite3').exists())
    ...
    def test_duplicate_dispatch_preserves_both_snapshots(self):
        self.recover(); before = {p.name: p.read_bytes() for p in self.root.iterdir()}
        with self.assertRaises(Exception): self.recover()
