...
def recover(expected_config, expected_bundle, evidence):
    ...
    try:
        ...
        try:
            ...
            fd = os.open('state_registry.recovery-d.sqlite3', os.O_WRONLY | os.O_CREAT | os.O_EXCL | os.O_NOFOLLOW, 0o600, dir_fd=parent)
            os.close(fd)
        finally:
            os.close(parent)
        PHASE = 'new_backup_created'
        snapshot = ROOT / 'state_registry.recovery-d.sqlite3'
        before = time.monotonic()
        with sqlite3.connect('file:' + str(DB) + '?mode=ro', uri=True, timeout=0) as source, sqlite3.connect(snapshot, timeout=0) as target:
            source.execute('PRAGMA query_only=ON')
...
def main(argv=None):
    try:
        ...
        parser.add_argument('--execute-approved-recovery-d', action='store_true')
        ...
        if not args.execute_approved_recovery_d:
