...
Offline reproduction: `python3 wtyj/scripts/verify_isluno_offline.py`. It denies sockets/DNS, uses a synthetic config and does not load the legacy pytest root fixtures. Tests exercise validation, source/sample separation, real-mode rejection, concurrent publication (one winner), stale edits, history/snapshot preservation, media injection rejection, parsing, inventory coverage and every retained asset hash/length. Result at this candidate: **17 tests passed**; every retained asset hash/length matched. Tool-only dependencies are pinned in `wtyj/tests/isluno/requirements.txt`. The baseline verifier remains separately available; no live model, WhatsApp, email, supplier or payment test is implied.
