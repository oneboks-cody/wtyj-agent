...
Owner approved a PDF URL card followed by a compact two-reply-button message and requested both messages on WhatsApp. Labels: Bekijk offerte / Akkoord, ga verder / Wijziging doorgeven, localized across six languages. Provider documentation confirms URL and reply buttons require separate messages. Send decisions only after the PDF card is delivered; persist each message separately in the existing card delivery ledger so retry does not duplicate the first card. Button approvals require an issued payload matching account, conversation, quote document and reservation revision. Obsolete buttons and review-frozen reservations cannot proceed. A change request asks what to change and blocks approval of the same document. Handler reuses the existing quote approval/payment path, without a model call for button clicks. No schema change.

Eleven offline tests passed with networking prohibited, covering quote review, button scope, change blocking, acceptance, and localized label lengths. Deployment approval requested before sending functional buttons; no sends yet. Rollback restores source/catalog together; stale button payloads must remain non-authoritative.

Owner approved deployment and sending both messages. Deployed `wtyj-agent:tracy-quote-buttons-4ee5389`: two source hashes and twelve catalog fields verified; health 200; 58 database tables unchanged during cutover and six peer containers unchanged. Backup `/root/backups/tracy-quote-buttons-4ee5389-20260907T145204Z`. Exact scoped candidate passed eleven offline tests.

Owner separately authorized returning the existing unpaid test quote to quote_ready and invalidating its old checkout link. Atomic tenant/account/conversation/revision checks verified no payment or human review; one link revoked, details/PDF preserved, revision advanced and owner-reset audit event recorded. Two requested WhatsApp messages then delivered to verified owner ending 003: PDF card followed by decision buttons. Enforced maximum two message POSTs, recorded delivery evidence and dashboard messages, idempotency key `tracy-quote-buttons-owner-20260907-v1`. No model calls, extra messages or email sends.

## Quote correction completion and document-language release
Owner requested fixing the audited correction gap and deploying the previously implemented six-language document selector. Keep corrected quotes on the same reservation: validate the draft, atomically version unpaid intake/pricing with account/conversation/revision checks, audit changed fields, invalidate prior quote approval, preserve old PDFs, render a versioned replacement, and require approval of that replacement. Date corrections before payment now use this route; booked-date changes retain their existing flow. Persist retry intent before rendering and reuse the corrected revision on retry. No paid-provider testing or customer sends authorized for this release.

27 focused offline tests passed for corrections, previous button scope/approval flow, and the selector (six choices, preserved chat language/profile, confirmation blocked until selection). Release includes existing selector implementation 164b765 as explicitly requested. No new schema required; document revision support already exists. Rollback restores source and config together while retaining audit/document history.

Authorized combined deployment completed: `wtyj-agent:tracy-demo-ready-f9cf824`. Exact live-overlay candidate passed 28 offline checks. Nine source hashes and two configuration fields verified; health 200, all 58 database tables unchanged during cutover and six peer containers unchanged. Backup `/root/backups/tracy-demo-ready-f9cf824-20260907T151611Z`. Live imports verified correction store/handler, six language options, and both active prompt policies; public dashboard responds 200. No paid model calls or customer sends. The previously pending document-language selector is now live. Remaining limitation: natural-language provider quality and end-to-end provider delivery were not exercised by this offline verification.
