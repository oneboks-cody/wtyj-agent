...
require(len(raw)<=400000,'bundle_limit')
with tarfile.open(fileobj=io.BytesIO(raw),mode='r:') as archive:
 members=archive.getmembers();require(len(members)==12 and {m.name for m in members}==set(MANIFEST['files']),'archive_members')
 files={}
...
print(json.dumps({'status':'staged','bundle_sha256':hashlib.sha256(raw).hexdigest(),'bytes':len(raw),'files':12}))
