...
class RunnerTests(unittest.TestCase):
 ...
 def test_atomic_profile_write_and_restore(self):
  with tempfile.TemporaryDirectory() as d:
   p=Path(d)/'profile.json';p.write_bytes(b'{"identity":"same"}');p.chmod(0o640)
   before=p.read_bytes();after=b'{"identity":"same","hospitality_voice":{}}';backup=Path(d)/'before.json'
   self.ns['replace_profile'](p,before,after,backup)
   self.assertEqual((p.read_bytes(),backup.read_bytes(),stat.S_IMODE(p.stat().st_mode)),(after,before,0o640))
   self.ns['replace_profile'](p,after,before,None);self.assertEqual(p.read_bytes(),before)
 def test_profile_drift_preserved(self):
  with tempfile.TemporaryDirectory() as d:
   p=Path(d)/'profile.json';p.write_bytes(b'fresh')
   with self.assertRaisesRegex(Rejected,'profile_changed'):self.ns['replace_profile'](p,b'stale',b'new',Path(d)/'backup')
   self.assertEqual(p.read_bytes(),b'fresh');self.assertFalse((Path(d)/'backup').exists())
