STAGE=Path('/root/isluno-hospitality-3a5f524-a')
BASE_IMAGE=BACKEND
CURRENT_CONTAINER='5acae4259714c41aeca3e310e6466cca27cfda19f0724d6439675c02e5c624b7'
TAG='isluno-local:hospitality-3a5f524'
...
s=m.status('/app/data/state_registry.db');assert s['phase']=='sealed' and s['generation']==7 and not s['workers'] and not s['pending'] and s['unreviewed_ledgers']==['isluno_conversation_turns','isluno_discovery_plans','isluno_recovery_incidents']
print(json.dumps({'ready':True,'catalog':31,'generation':7}))
"""
 require(runtime(code)['ready'],'readiness_failed')
 proof=runtime(OPERATOR_CODE+'\nprint(json.dumps(readiness(\'/app/data/state_registry.db\',7,'+repr(PROOF_HASH)+')))')
...
def reopen(icp,worker_fd,watchdog_fd):
 runtime(OPERATOR_CODE+'\nreopen(\'/app/data/state_registry.db\',7,'+repr(PROOF_HASH)+');print(json.dumps({\'open\':True}))')
 gate=json.loads(cmd(['docker','exec',icp['Id'],'python','-m','host.mermaid_ingress_control','reopen','--execute-approved-action','--expected-generation','6','--runtime-ready-verified'],15));require(gate['phase']=='open' and gate['generation']==7,'ingress_reopen')
 marker=Path('/root/clients/mermaid/.maintenance');require(checked_read(marker,100)==b'isluno-hospitality-3a5f524-a\n','marker_changed');marker.unlink()
...
def replace_profile(path,expected,replacement,backup):
 require(checked_read(path,65536)==expected,'profile_changed')
 st=os.stat(path,follow_symlinks=False);require(stat.S_ISREG(st.st_mode) and st.st_nlink==1,'profile_shape')
 if backup is not None:
  fd=os.open(backup,os.O_WRONLY|os.O_CREAT|os.O_EXCL|os.O_NOFOLLOW,0o600)
  with os.fdopen(fd,'wb') as f:f.write(expected);f.flush();os.fsync(f.fileno())
 temporary=path.with_name(path.name+'.hospitality-tmp')
 fd=os.open(temporary,os.O_WRONLY|os.O_CREAT|os.O_EXCL|os.O_NOFOLLOW,stat.S_IMODE(st.st_mode))
 with os.fdopen(fd,'wb') as f:
  os.fchown(f.fileno(),st.st_uid,st.st_gid);f.write(replacement);f.flush();os.fsync(f.fileno())
 require(checked_read(path,65536)==expected,'profile_raced')
...
def execute():
 ...
 require(hashlib.sha256(profile_before).hexdigest()=='1be5a6297fd2fe3e5609214925a220c1b80598eb7a5f031d8af8caa80b2fce58','profile_binding')
 current_profile=json.loads(profile_before);patch=json.loads(checked_read(STAGE/'profile-patch.json',65536))
 require(set(patch)=={'hospitality_voice','conversation_outcomes'},'profile_patch_scope')
 require(current_profile['tenant_slug']=='mermaid' and current_profile['brand']['id']=='isluno' and current_profile['mode']=='demo','profile_identity')
 merged={**current_profile,**patch}
 require({k:v for k,v in merged.items() if k not in patch}=={k:v for k,v in current_profile.items() if k not in patch},'profile_unrelated_change')
 profile_after=(json.dumps(merged,ensure_ascii=False,indent=2)+'\n').encode()
 profile_hash=hashlib.sha256(profile_after).hexdigest()
 ...
 base=image_info('isluno-local:no-reply-c04512c');require(base['Id']==BASE_IMAGE,'base_image_tag')
 ...
 require(candidate['Config'].get('Labels',{}).get('org.opencontainers.image.revision')=='3a5f5240f9d51e71fb47c47fbce8a16d1688f78e','source_revision')
 ...
 gate=json.loads(cmd(['docker','exec',icp['Id'],'python','-m','host.mermaid_ingress_control'],10));require(gate['phase']=='open' and gate['generation']==5,'ingress_state')
 ...
 fd=os.open(marker,os.O_WRONLY|os.O_CREAT|os.O_EXCL,0o600);os.write(fd,b'isluno-hospitality-3a5f524-a\n');os.close(fd);STATE['marker_created']=True
 ...
 gate=json.loads(cmd(['docker','exec',icp['Id'],'python','-m','host.mermaid_ingress_control','close','--execute-approved-action','--expected-generation','5'],20));require(gate['phase']=='closed' and gate['generation']==6,'ingress_close')
 runtime("import json\nfrom shared import mermaid_maintenance as m\nassert m.close(6,seconds=120,coverage=m.COVERAGE,evidence='reviewed hospitality image and two-field profile rollout',db_path='/app/data/state_registry.db')==7\nprint(json.dumps({'closed':True}))")
 ...
 runtime(OPERATOR_CODE+'\nseal(\'/app/data/state_registry.db\',7,'+repr(PROOF_HASH)+');print(json.dumps({\'sealed\':True}))')
 STATE['sealed']=True
 ...
 try:
  replace_profile(profile_path,profile_before,profile_after,STAGE/'profile-before.json')
  ...
  require(actual_profile in (profile_before,profile_after),'rollback_profile_unknown')
 ...
 reopen(icp,worker_fd,watchdog_fd)
 return {'status':'fix_deployed_open','image':candidate_id,'container_id':fresh['Id'],'source':'3a5f5240f9d51e71fb47c47fbce8a16d1688f78e','customer_tables_preserved':len(rows),'runtime_generation':8,'ingress_generation':7,'profile_sha256':profile_hash,'agent_messages_sent':0,'health':200,'state':STATE}
...
def abort_before_activation():
 ...
 require(digest(CLIENT.with_name('isluno_profile.json'))=='1be5a6297fd2fe3e5609214925a220c1b80598eb7a5f031d8af8caa80b2fce58','abort_profile_changed')
 ...
 if state['phase']=='draining' and state['generation']==7:
  runtime(OPERATOR_CODE+'\nabort_drain(\'/app/data/state_registry.db\',7,'+repr(PACKET_SHA)+');print(json.dumps({\'aborted\':True}))')
 else:require(state['phase']=='open' and state['generation']==6,'abort_unknown_phase')
 ...
 if gate['phase']=='closed' and gate['generation']==6:
  gate=json.loads(cmd(['docker','exec',icp['Id'],'python','-m','host.mermaid_ingress_control','reopen','--execute-approved-action','--expected-generation','6','--runtime-ready-verified'],15))
  require(gate['phase']=='open' and gate['generation']==7,'abort_ingress_reopen')
 else:require(gate['phase']=='open' and gate['generation']==5,'abort_unknown_ingress')
...
def restore_original_after_pre_activation_failure():
 ...
 require(digest(CLIENT.with_name('isluno_profile.json'))=='1be5a6297fd2fe3e5609214925a220c1b80598eb7a5f031d8af8caa80b2fce58','restore_profile_changed')
