...
class OperatorTests(unittest.TestCase):
 def setUp(self):
  ...
  c=sqlite3.connect(self.path);c.executescript(m.SCHEMA+"CREATE TABLE isluno_cutover(tenant TEXT PRIMARY KEY,activated_at TEXT NOT NULL); INSERT INTO isluno_cutover VALUES('mermaid','fixture'); INSERT INTO mermaid_maintenance VALUES('mermaid',6,'open',0,'[]','');");c.close()
 ...
 def drain(self):m.close(6,seconds=120,coverage=m.COVERAGE,evidence='fixture',db_path=self.path)
 def test_seal_readiness_reopen_preserve_rows(self):
  before=self.proof();self.drain()
  with self.assertRaises(m.Closed):m.seal(7,db_path=self.path)
  operator.seal(self.path,7,before['sha256']);self.assertEqual(operator.readiness(self.path,7,before['sha256']),before)
  operator.reopen(self.path,7,before['sha256']);self.assertEqual(self.proof(),before)
  self.assertEqual((m.status(self.path)['phase'],m.status(self.path)['generation']),('open',8))
 ...
 def test_stale_hash_does_not_seal(self):
  self.drain()
  with self.assertRaisesRegex(ValueError,'stale_history_proof'):operator.seal(self.path,7,'0'*64)
  self.assertEqual(m.status(self.path)['phase'],'draining')
 def test_uncertainty_active_work_wrong_binding_rejected(self):
  mutations=["UPDATE isluno_discovery_plans SET status='ambiguous'", "UPDATE isluno_discovery_plans SET trigger_id='unrelated'", "UPDATE inbound_processing_events SET processing_token='active' WHERE message_id='complete'", "UPDATE isluno_recovery_incidents SET status='open'", "INSERT INTO mermaid_maintenance_workers VALUES('worker',6,'inbound','active',1,NULL,'')", "UPDATE isluno_booking_sessions SET payload=json_set(payload,'$.active_itinerary_id','unknown')"]
 ...
 def test_abort_unsealed_drain_preserves_unfinished_work(self):
  before=self.proof();self.drain()
  c=sqlite3.connect(self.path);c.execute("INSERT INTO mermaid_maintenance_workers VALUES('worker',7,'inbound','active',1,NULL,'')");c.commit();c.close()
  operator.abort_drain(self.path,7,'fixture-packet')
 ...
 def test_abort_cannot_reopen_sealed_or_wrong_generation(self):
  ...
  with self.assertRaises(ValueError):operator.abort_drain(self.path,6,'fixture')
  operator.seal(self.path,7,proof['sha256'])
  with self.assertRaises(ValueError):operator.abort_drain(self.path,7,'fixture')
