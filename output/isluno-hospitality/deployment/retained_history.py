...
def review(db):
    ...
    check(len(turns)==2 and len(contacts)==1,'different_history_shape')
    check(len(data['isluno_recovery_incidents'])==len(data['isluno_booking_sessions'])==len(data['isluno_discovery_plans'])==len(data['isluno_discovery_latest'])==1,'different_history_shape')
    ...
    failed=[t for t in turns if t['decision'] is None and t['outcome'] is None]
    complete=[t for t in turns if t['decision'] is not None and t['outcome'] is not None]
    check(len(failed)==len(complete)==1,'unfinished_or_extra_turn')
    ...
    incident=data['isluno_recovery_incidents'][0]
    check(incident['scope_key']==scope.key and incident['trigger_id']==failed[0]['trigger_id'],'incident_scope')
    check(incident['kind']=='understanding_failure' and incident['status']=='progress_resumed_new_turn' and incident['code']=='ItineraryError','unreviewed_incident')
    check(incident['itinerary_id'] is None,'unexpected_itinerary')
    session_row=data['isluno_booking_sessions'][0]
    check(session_row['scope_key']==scope.key,'session_scope')
    ...
    check(len(current)==2 and {r['message_id'] for r in current}=={t['trigger_id'] for t in turns},'inbound_link')
    for row in current:
        ...
        check(row['provider_retry_count']==0,'provider_retry_present')
        if row['message_id']==failed[0]['trigger_id']:
            check((row['status'],row['reason'])==('ignored','no_reply_returned'),'failed_turn_disposition')
            check(row['outbound_attempted_at']=='' and row['attempt_count']==0,'failed_turn_attempt')
        else:
            # This sender owns attempt/acceptance in isluno_discovery_plans, not
            # the generic text-send fields. Fresh metadata proves those unused.
            check(row['attempt_count']==0 and row['outbound_attempted_at']=='' and row['outbound_idempotency_key']=='','successful_attempt_metadata')
            check((row['status'],row['reason'])==('replied','provider_send_ok'),'successful_turn_disposition')
    ...
    plan_row=data['isluno_discovery_plans'][0];plan=json.loads(plan_row['payload'])
    from agents.social.isluno_itinerary import encoded
    expected_trigger='conversation-'+hashlib.sha256(encoded([scope.key,complete[0]['trigger_id'],'reply']).encode()).hexdigest()[:32]
    check(plan_row['id']==plan['id'] and plan_row['trigger_id']==plan['trigger_id']==expected_trigger,'successful_plan_trigger')
    ...
    check(data['isluno_discovery_latest'][0]['scope_key']==scope.key and data['isluno_discovery_latest'][0]['plan_id']==plan['id'],'latest_plan_binding')
    check(all(a['plan_id']==plan['id'] for a in data['isluno_discovery_actions']),'unbound_native_action')
    check(len(data['isluno_recovery_inbound'])==2 and all(r['scope_key']==scope.key for r in data['isluno_recovery_inbound']),'recovery_link')
    ...
    check(len(raw)<=4000000,'proof_bytes_bound')
    return {'sha256':hashlib.sha256(raw).hexdigest(),'classification':'one_failed_claim_retained_one_completed_browsing_turn_one_accepted_plan',
            'turns':2,'accepted_plans':1,'unreviewed_ledgers':UNKNOWN}
...
def seal(path,generation,expected):
    ...
    try:
        ...
        db.execute('INSERT INTO mermaid_maintenance_audit(generation,action,at) VALUES(?,?,?)',(generation,'seal_hospitality_history:'+expected,time.time()))
...
def readiness(path,generation,expected):
    ...
    try:
        ...
        proof=verify(db,expected)
        check(db.execute('SELECT count(*) FROM mermaid_maintenance_audit WHERE generation=? AND action=?',(generation,'seal_hospitality_history:'+expected)).fetchone()[0]==1,'seal_audit_missing')
        return proof
    finally:db.close()
...
def reopen(path,generation,expected):
    ...
    try:
        ...
        db.execute('INSERT INTO mermaid_maintenance_audit(generation,action,at) VALUES(?,?,?)',(generation,'reopen_hospitality_history:'+expected,time.time()))
        db.commit();return proof
    except BaseException:db.rollback();raise
    finally:db.close()
...
def abort_drain(path,generation,packet):
    ...
    try:
        ...
        db.execute("UPDATE mermaid_maintenance SET phase='open',generation=generation+1,coverage='[]',evidence='' WHERE tenant='mermaid'")
        db.execute('INSERT INTO mermaid_maintenance_audit(generation,action,at) VALUES(?,?,?)',(generation,'abort_hospitality_before_cutover:'+packet,time.time()))
        db.commit()
    except BaseException:db.rollback();raise
    finally:db.close()
