...
code=OPERATOR_CODE+"\ndb=sqlite3.connect('file:/app/data/state_registry.db?mode=ro',uri=True,timeout=0)\ntry:\n db.execute('PRAGMA query_only=ON');db.execute('BEGIN');s=maintenance._snapshot(db);check(s['phase']=='open' and s['generation']==10,'preflight_generation');proof=verify(db,"+repr(PROOF_HASH)+");print(json.dumps({'status':'preflight_pass','proof':proof}))\nfinally:db.close()\n"
