STAGE=Path('/root/isluno-visual-0361641-a')
BASE_IMAGE=BACKEND
CURRENT_CONTAINER='d698b4d8daa5ad58fbdc1de4a13a9a75a046e6dc13bff5e73624199b53d47950'
TAG='isluno-local:visual-0361641'
...
DASHBOARD_TARGET='/var/www/unboks-dashboard/releases/isluno-25dfebf-20260909-b'

def dashboard_binding():
 require(DASHBOARD.is_symlink() and os.readlink(DASHBOARD)==DASHBOARD_TARGET,'dashboard_pointer_changed')
 require(digest(Path(DASHBOARD_TARGET)/'index.html')=='c15c71d8487780a63999578005560899a50634e4c8f4a1b2a9e9419093c9f11b','dashboard_index_changed')


def catalog_binding():
 old=json.loads(checked_read(STAGE/'catalog-before.json',1048576));new=json.loads(checked_read(STAGE/'catalog.json',1048576))
 approved={'klein-curacao-a-trip-to-paradise','klein-curacao-catamaran-day-trip','klein-curacao-weekly-luxury-yacht-trip','miss-ann-klein-curacao','all-west-beach-hopping','snorkel-and-beach-adventures','atv-buggy-3-hours'}
 require(new['version']=='isluno-demo-v3-visual-context-20260909','catalog_version')
 restored=json.loads(json.dumps(new));restored['version']=old['version'];changed=set()
 for product in restored['products']:
  if product['id'] in approved:
   context=product.get('source_claims',{}).pop('location_context',None)
   require(isinstance(context,dict) and set(context)=={'text','source_url','evidence_kind','observed_on'} and context['source_url'].startswith('https://isluno.com/'),'catalog_context')
   changed.add(product['id'])
 require(changed==approved and restored==old,'catalog_unrelated_change')


def select_catalog(restore=False):
 target=CLIENT.with_name('isluno_catalog.json')
 before=MANIFEST['files']['catalog-before.json']['sha256'];after=MANIFEST['files']['catalog.json']['sha256']
 expected=after if restore else before;desired=before if restore else after
 current=digest(target)
 if restore and current==before:
  STATE['catalog_changed']=False;return
 require(current==expected,'catalog_changed_outside_release')
 source=STAGE/('catalog-before.json' if restore else 'catalog.json');data=checked_read(source,1048576)
 require(hashlib.sha256(data).hexdigest()==desired,'catalog_source_hash')
 temporary=target.with_name('isluno_catalog.visual-rollback.json' if restore else 'isluno_catalog.visual-release.json')
 fd=os.open(temporary,os.O_WRONLY|os.O_CREAT|os.O_EXCL|os.O_NOFOLLOW,stat.S_IMODE(target.stat().st_mode))
 with os.fdopen(fd,'wb') as f:f.write(data);f.flush();os.fsync(f.fileno())
 require(digest(target)==expected,'catalog_changed_before_swap')
 os.replace(temporary,target)
 STATE['catalog_changed']=not restore
 parent=os.open(target.parent,os.O_RDONLY|os.O_DIRECTORY)
 try:os.fsync(parent)
 finally:os.close(parent)
 require(digest(target)==desired,'catalog_swap_hash')
...
s=m.status('/app/data/state_registry.db');assert s['phase']=='sealed' and s['generation']==13 and not s['workers'] and not s['pending'] and s['unreviewed_ledgers']==['isluno_conversation_turns','isluno_discovery_plans','isluno_recovery_incidents']
print(json.dumps({'ready':True,'catalog':31,'generation':13}))
"""
 require(runtime(code)['ready'],'readiness_failed')
 proof=runtime(OPERATOR_CODE+'\nprint(json.dumps(readiness(\'/app/data/state_registry.db\',13,'+repr(PROOF_HASH)+')))')
 ...
 if check_source:
  ...
  entrypoint='from agents.social import webhook_server,isluno_callbacks,isluno_quote_delivery,isluno_visual\nfrom agents.social.isluno_wire import messages,validate_body\nassert callable(webhook_server.receive_zernio_webhook) and callable(isluno_callbacks.accept) and callable(isluno_quote_delivery.send_job)\nassert any(r.path=="/webhooks/zernio" for r in webhook_server.app.routes)\nparts=messages({"accountId":"offline-readiness","message":"A"*1122,"buttons":[{"type":"postback","title":"Select","payload":"fixture"}]})\nassert len(parts)==2\nfor part in parts:validate_body(part)\nprint("entrypoints_ready")'
  require(cmd(['docker','exec',fresh['Id'],'python','-c',entrypoint],15).strip()=='entrypoints_ready','candidate_entrypoints')
  context_check="import json;from shared.isluno_catalog import CatalogStore;from agents.social.isluno_understanding import facts;from shared import config_loader;from pathlib import Path;c=CatalogStore(Path('/app/config/isluno_catalog.json')).snapshot();assert sum('location_context' in facts(p) for p in c['catalog']['products'])==7;assert (config_loader.get_raw().get('isluno') or {}).get('native_carousels') is False;print('visual_ready')"
  require(cmd(['docker','exec',fresh['Id'],'python','-c',context_check],10).strip()=='visual_ready','visual_catalog_runtime')
  require(protected_rows()==rows,'readiness_changed_customer_data')
 return fresh


def reopen(icp,worker_fd,watchdog_fd):
 runtime(OPERATOR_CODE+'\nreopen(\'/app/data/state_registry.db\',13,'+repr(PROOF_HASH)+');print(json.dumps({\'open\':True}))')
 gate=json.loads(cmd(['docker','exec',icp['Id'],'python','-m','host.mermaid_ingress_control','reopen','--execute-approved-action','--expected-generation','12','--runtime-ready-verified'],15));require(gate['phase']=='open' and gate['generation']==13,'ingress_reopen')
 marker=Path('/root/clients/mermaid/.maintenance');require(checked_read(marker,100)==b'isluno-visual-0361641-a\n','marker_changed');marker.unlink()
...
def execute():
 ...
 catalog_binding()
 require(digest(CLIENT.with_name('isluno_catalog.json'))==MANIFEST['files']['catalog-before.json']['sha256'],'catalog_before_binding')
 ...
 base=image_info('isluno-local:communication-ab88343');require(base['Id']==BASE_IMAGE,'base_image_tag')
 ...
 require(candidate['Config'].get('Labels',{}).get('org.opencontainers.image.revision')=='0361641575552e5a78a671efbc1dd59b9d5196ba','source_revision')
 ...
 catalog_check="import json;from shared.isluno_catalog import _validate_catalog;_validate_catalog(json.load(open('/opt/isluno-catalog-candidate.json')));print('catalog_valid')"
 require(cmd(['docker','run','--rm','--pull','never','--network','none','--entrypoint','python',candidate_id,'-c',catalog_check],15).strip()=='catalog_valid','candidate_catalog')
 ...
 gate=json.loads(cmd(['docker','exec',icp['Id'],'python','-m','host.mermaid_ingress_control'],10));require(gate['phase']=='open' and gate['generation']==11,'ingress_state')
 ...
 fd=os.open(marker,os.O_WRONLY|os.O_CREAT|os.O_EXCL,0o600);os.write(fd,b'isluno-visual-0361641-a\n');os.close(fd);STATE['marker_created']=True
 ...
 gate=json.loads(cmd(['docker','exec',icp['Id'],'python','-m','host.mermaid_ingress_control','close','--execute-approved-action','--expected-generation','11'],20));require(gate['phase']=='closed' and gate['generation']==12,'ingress_close')
 runtime("import json\nfrom shared import mermaid_maintenance as m\nassert m.close(12,seconds=120,coverage=m.COVERAGE,evidence='reviewed communication and operator visibility rollout',db_path='/app/data/state_registry.db')==13\nprint(json.dumps({'closed':True}))")
 ...
 runtime(OPERATOR_CODE+'\nseal(\'/app/data/state_registry.db\',13,'+repr(PROOF_HASH)+');print(json.dumps({\'sealed\':True}))')
 STATE['sealed']=True
 ...
 try:
  select_catalog();protected[CLIENT.with_name('isluno_catalog.json')]=MANIFEST['files']['catalog.json']['sha256']
  fresh=switch(candidate_id,STAGE/'release.yml');STATE['image_changed']=True
  fresh=ready(candidate_id,mm,rows,protected,True)
  require(protected_rows()==rows,'catalog_readiness_changed_customer_data')
  event('visual_catalog_ready',catalog_sha256=MANIFEST['files']['catalog.json']['sha256'])
  ...
  select_catalog(restore=True);protected[CLIENT.with_name('isluno_catalog.json')]=MANIFEST['files']['catalog-before.json']['sha256']
 ...
 reopen(icp,worker_fd,watchdog_fd)
 return {'status':'fix_deployed_open','image':candidate_id,'container_id':fresh['Id'],'source':'0361641575552e5a78a671efbc1dd59b9d5196ba','customer_tables_preserved':len(rows),'runtime_generation':14,'ingress_generation':13,'profile_sha256':hashlib.sha256(profile_before).hexdigest(),'agent_messages_sent':0,'health':200,'frontend_commit':MANIFEST['frontend_commit'],'frontend_target':DASHBOARD_TARGET,'frontend_unchanged':True,'catalog_sha256':MANIFEST['files']['catalog.json']['sha256'],'state':STATE}
...
def abort_before_activation():
 ...
 require(not STATE['image_changed'] and not STATE.get('profile_changed') and not STATE.get('catalog_changed') and not STATE['sealed'],'abort_after_cutover')
 mm=inspect('wtyj-mermaid');icp=inspect('unboks-internal-control-panel-wtyj-admin-1')
 ...
 if state['phase']=='draining' and state['generation']==13:
  runtime(OPERATOR_CODE+'\nabort_drain(\'/app/data/state_registry.db\',13,'+repr(PACKET_SHA)+');print(json.dumps({\'aborted\':True}))')
 else:require(state['phase']=='open' and state['generation']==12,'abort_unknown_phase')
 ...
 if gate['phase']=='closed' and gate['generation']==12:
  gate=json.loads(cmd(['docker','exec',icp['Id'],'python','-m','host.mermaid_ingress_control','reopen','--execute-approved-action','--expected-generation','12','--runtime-ready-verified'],15))
  require(gate['phase']=='open' and gate['generation']==13,'abort_ingress_reopen')
 else:require(gate['phase']=='open' and gate['generation']==11,'abort_unknown_ingress')
...
def restore_original_after_pre_activation_failure():
 require(not STATE['image_changed'] and not STATE.get('profile_changed') and not STATE.get('catalog_changed'),'restore_after_activation')
