...
r=ROOT/'tmp/isluno-visual-rollout'
...
try:result=execute()
 ...
 if any(STATE.get(k) for k in ('watchdog_stopped','worker_stopped','marker_created','gates_closed','sealed')) and not STATE.get('image_changed') and not STATE.get('profile_changed') and not STATE.get('catalog_changed'):
...
packet={'source_commit':manifest['source_commit'],'frontend_commit':manifest['frontend_commit'],'preflight_sha256':hashlib.sha256(preflight.encode()).hexdigest(),'stage_sha256':hashlib.sha256(stage.encode()).hexdigest(),'rollout_template_sha256':hashlib.sha256(rollout.encode()).hexdigest(),'bundle':manifest['bundle'],'history_proof':proof,'overall_seconds':900,'transfers':1,'transfer_seconds':60,'build_seconds':120,'minimum_before_disruption_seconds':500,'minimum_before_activation_seconds':300,'rollback_reserve_seconds':180,'no_automatic_retries':True,'postverification_reserve_seconds':180,'postverification_script_sha256':hashlib.sha256((r/'verify_deployment.py').read_bytes()).hexdigest(),'frontend_target':'/var/www/unboks-dashboard/releases/isluno-25dfebf-20260909-b','frontend_unchanged':True,'catalog_before_sha256':manifest['files']['catalog-before.json']['sha256'],'catalog_after_sha256':manifest['files']['catalog.json']['sha256']}
