...
class OperatorTests(unittest.TestCase):
 def setUp(self):
  ...
  rejected=('rejected-one','rejected-two')
  for index,trigger in enumerate(rejected,3):
   state_registry.wa_claim_inbound_processing(trigger,self.t.scope().conversation_id,'whatsapp',payload={'text':'synthetic'})
   reply,_=self.t.turn(conversation.response(products=[],fact_keys=[]),trigger=trigger)
   c=sqlite3.connect(self.path);c.execute("UPDATE isluno_discovery_plans SET status='rejected',provider_id=NULL WHERE id=?",(reply['media']['url'],));c.commit();c.close()
   state_registry.inbound_processing_bulk_update([trigger],'send_failed',reason='provider_send_failed',error='synthetic-failure')
  trigger='latest-accepted';self.t.discovery.clock=lambda:conversation.NOW+timedelta(seconds=40)
  state_registry.wa_claim_inbound_processing(trigger,self.t.scope().conversation_id,'whatsapp',payload={'text':'synthetic'})
  latest,_=self.t.turn(conversation.response(products=[],fact_keys=[]),trigger=trigger)
  self.assertTrue(send_plan(self.t.scope().conversation_id,self.t.scope().account_id,latest['media']['url'],store=self.t.discovery,post=lambda *a:{'status':'accepted','provider_id':'fixture-latest'},window=lambda *a:{'open':True},sleep=lambda *a:None))
  state_registry.inbound_processing_bulk_update([trigger],'replied',reason='provider_send_ok')
  for index in range(174):
   ident='retained-'+str(index);state_registry.wa_claim_inbound_processing(ident,self.t.scope().conversation_id,'whatsapp',payload={'text':'synthetic'})
   state_registry.inbound_processing_bulk_update([ident],'ignored',reason='history_cleared_no_replay')
  c=sqlite3.connect(self.path)
  c.execute("UPDATE isluno_recovery_incidents SET status='progress_resumed_new_turn'")
  for index in range(2):
   row=c.execute("INSERT INTO pending_notifications(notification_type,channel,customer_id,subject,body,created_at,mode) VALUES('escalation','whatsapp',?,'[DELIVERY FAILED] Automated reply needs review','synthetic','fixture','soft')",(self.t.scope().conversation_id,))
   c.execute('INSERT INTO inbound_operator_notifications VALUES(?,?,?)',('fixture-'+str(index),row.lastrowid,'fixture'))
  c.commit()
  data={table:operator.records(c,table) for table in operator.TABLES}
  schema={table:c.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name=?",(table,)).fetchone()[0] for table in operator.TABLES};c.close()
  for key,value in [('BASE_HASH',operator.digest({'schema':schema,'rows':data})),('REJECTED',{hashlib.sha256(t.encode()).hexdigest() for t in rejected}),('FAILURE_ERROR_HASH',hashlib.sha256(b'synthetic-failure').hexdigest())]:
   patcher=patch.object(operator,key,value);patcher.start();self.addCleanup(patcher.stop)
 def proof(self):
  c=sqlite3.connect(self.path)
