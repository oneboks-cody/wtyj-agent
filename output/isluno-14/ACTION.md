...
`required-inputs.json` is the complete target-binding record with explicit nulls. Requested from Calvin: a sanitized inventory containing current host/project/platform, backend image/source and service/compose/config/database paths, frontend current source/release/pointer/API route, intended media/document bases, exact account and ownership/sole-consumer evidence. Also required before final action approval: ingress retention/drain mechanism, approved backup location/verification, protected config CAS digest, change window and the final OCI digest. Supply metadata or access to an already authorized sanitized inventory; do not send API keys, passwords or customer records. Provider calls or privileged production inspection are not authorized by this request.
