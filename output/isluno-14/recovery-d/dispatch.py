...
BASE = REPO/'output/isluno-14/recovery-d'
PRIVATE = REPO/'tmp/isluno-recovery-d'
...
def prepare():
    ...
    raw=(REPO/'wtyj/scripts/recover_isluno_staged_cutover.py').read_bytes()
    ...
    bindings={'EXPECTED':expected,'STOPPED_ICP_ID':icp_id,'STAGED_CONFIG_SHA':inspection['config_sha256'],
              ...
              'APPROVAL_REFERENCE':'calvin-isluno-recovery-d'}
    code=prefix+'\n'+safe+'\n'+'\n'.join(key+'='+repr(value) for key,value in bindings.items())+'\n'+(BASE/'remote.py').read_text()+'''
def alarm(*_):raise Rejected('recovery_d_deadline')
...
'''
    ...
    compile(code,'recovery-d-exact-payload','exec')
    durable(PRIVATE/'exact-payload.py',payload)
    files=['wtyj/scripts/recover_isluno_staged_cutover.py','wtyj/scripts/inspect_isluno_target.py','wtyj/tests/isluno/test_staged_recovery.py','output/isluno-14/RECOVERY-D.md','output/isluno-14/recovery-d/remote.py','output/isluno-14/recovery-d/dispatch.py']
    packet={'run_id':'calvin-isluno-recovery-d','payload_sha256':hashlib.sha256(payload).hexdigest(),'payload_bytes':len(payload),
...
def dispatch(approval_file):
    ...
    assert approval.get('approved_recovery_d') is True
...
if __name__=='__main__':
    ...
    group.add_argument('--prepare-only',action='store_true');group.add_argument('--execute-approved-recovery-d',action='store_true')
    parser.add_argument('--approval-file')
    args=parser.parse_args()
    if args.prepare_only:prepare()
    elif args.execute_approved_recovery_d:
