STAGE=Path('/root/isluno-first-contact-b0eef7a-a')
BASE_IMAGE=BACKEND
CURRENT_CONTAINER='011306f287c534df70209c09be21ad99954312fef93f0a3c5435274dae3e9a13'
TAG='isluno-local:first-contact-b0eef7a'
...
s=m.status('/app/data/state_registry.db');assert s['phase']=='sealed' and s['generation']==17 and not s['workers'] and not s['pending'] and s['unreviewed_ledgers']==[]
print(json.dumps({'ready':True,'catalog':31,'generation':17}))
"""
 require(runtime(code)['ready'],'readiness_failed')
 proof=runtime(OPERATOR_CODE+'\nprint(json.dumps(readiness(\'/app/data/state_registry.db\',17,'+repr(PROOF_HASH)+')))')
...
def reopen(icp,worker_fd,watchdog_fd):
 runtime(OPERATOR_CODE+'\nreopen(\'/app/data/state_registry.db\',17,'+repr(PROOF_HASH)+');print(json.dumps({\'open\':True}))')
 gate=json.loads(cmd(['docker','exec',icp['Id'],'python','-m','host.mermaid_ingress_control','reopen','--execute-approved-action','--expected-generation','16','--runtime-ready-verified'],15));require(gate['phase']=='open' and gate['generation']==17,'ingress_reopen')
 marker=Path('/root/clients/mermaid/.maintenance');require(checked_read(marker,100)==b'isluno-first-contact-b0eef7a-a\n','marker_changed');marker.unlink()
...
def execute():
 ...
 require(digest(CLIENT.with_name('isluno_catalog.json'))==MANIFEST['catalog_sha256'],'catalog_before_binding')
 ...
 base=image_info('isluno-local:visual-0361641');require(base['Id']==BASE_IMAGE,'base_image_tag')
 ...
 require(candidate['Config'].get('Labels',{}).get('org.opencontainers.image.revision')=='b0eef7a395d2ada58aa0e6e090b1b9dbfdbec420','source_revision')
 ...
 gate=json.loads(cmd(['docker','exec',icp['Id'],'python','-m','host.mermaid_ingress_control'],10));require(gate['phase']=='open' and gate['generation']==15,'ingress_state')
 ...
 fd=os.open(marker,os.O_WRONLY|os.O_CREAT|os.O_EXCL,0o600);os.write(fd,b'isluno-first-contact-b0eef7a-a\n');os.close(fd);STATE['marker_created']=True
 ...
 gate=json.loads(cmd(['docker','exec',icp['Id'],'python','-m','host.mermaid_ingress_control','close','--execute-approved-action','--expected-generation','15'],20));require(gate['phase']=='closed' and gate['generation']==16,'ingress_close')
 runtime("import json\nfrom shared import mermaid_maintenance as m\nassert m.close(16,seconds=120,coverage=m.COVERAGE,evidence='reviewed communication and operator visibility rollout',db_path='/app/data/state_registry.db')==17\nprint(json.dumps({'closed':True}))")
 STATE['gates_closed']=True
 # Sealing polls only read status; failures/unresolved rows are never erased or relabelled.
 end=time.monotonic()+20
 while True:
  status=runtime("import json\nfrom shared import mermaid_maintenance as m\ns=m.status('/app/data/state_registry.db');print(json.dumps({'ready':not s['workers'] and not s['pending'] and s['unreviewed_ledgers']==[]}))",10)
  if status['ready']:break
  require(time.monotonic()<end,'drain_not_complete');time.sleep(.2)
 runtime(OPERATOR_CODE+'\nseal(\'/app/data/state_registry.db\',17,'+repr(PROOF_HASH)+');print(json.dumps({\'sealed\':True}))')
 STATE['sealed']=True
 ...
 try:
  ...
  event('first_contact_ready',catalog_sha256=MANIFEST['catalog_sha256'])
 ...
 reopen(icp,worker_fd,watchdog_fd)
 return {'status':'fix_deployed_open','image':candidate_id,'container_id':fresh['Id'],'source':'b0eef7a395d2ada58aa0e6e090b1b9dbfdbec420','customer_tables_preserved':len(rows),'runtime_generation':18,'ingress_generation':17,'profile_sha256':hashlib.sha256(profile_before).hexdigest(),'agent_messages_sent':0,'health':200,'frontend_commit':MANIFEST['frontend_commit'],'frontend_target':DASHBOARD_TARGET,'frontend_unchanged':True,'catalog_sha256':MANIFEST['catalog_sha256'],'state':STATE}
...
def abort_before_activation():
 ...
 if state['phase']=='draining' and state['generation']==17:
  runtime(OPERATOR_CODE+'\nabort_drain(\'/app/data/state_registry.db\',17,'+repr(PACKET_SHA)+');print(json.dumps({\'aborted\':True}))')
 else:require(state['phase']=='open' and state['generation']==16,'abort_unknown_phase')
 ...
 if gate['phase']=='closed' and gate['generation']==16:
  gate=json.loads(cmd(['docker','exec',icp['Id'],'python','-m','host.mermaid_ingress_control','reopen','--execute-approved-action','--expected-generation','16','--runtime-ready-verified'],15))
  require(gate['phase']=='open' and gate['generation']==17,'abort_ingress_reopen')
 else:require(gate['phase']=='open' and gate['generation']==15,'abort_unknown_ingress')
